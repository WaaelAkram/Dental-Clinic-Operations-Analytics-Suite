...
class SelectDailyBatch extends Command
{
    ...
    // In app/Console/Commands/Marketing/SelectDailyBatch.php
...
public function handle(ClinicPatientGateway $gateway): int
{
    ...
    if ($lapsedCandidates->isEmpty()) {
        $this->info("No potentially lapsed patients found. Exiting.");
        return self::SUCCESS;
    }
    $this->info("Found {$lapsedCandidates->count()} potential candidates based on last appointment date.");

    $candidateIds = $lapsedCandidates->pluck('id')->all();
    $patientsWithFutureAppointments = $gateway->getPatientsWithFutureAppointments($candidateIds);

    if ($patientsWithFutureAppointments->isNotEmpty()) {
        $this->info("Found {$patientsWithFutureAppointments->count()} candidates with future appointments. They will be excluded.");
    }
    ...
    $doNotContactIds = $permanentExclusions
        ->merge($recentMessages)
        ->merge($patientsWithFutureAppointments)
        ->unique();
